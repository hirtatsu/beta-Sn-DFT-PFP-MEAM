"""
06_make_dft_slabs_local.py (run locally on WSL)

Build β-Sn slabs using the DFT/PBE-optimized bulk lattice (a=5.970, c=3.218 Å)
and write OpenMX inputs directly to supercomputer_inputs/.
"""
import json
import math
import os
...
BASE = os.path.expanduser('~/projects/beta-sn-elasticity/surface_gb/supercomputer_inputs')
...
def main():
    ...
    manifest = {
        'note': 'β-Sn slabs built on DFT/PBE-optimized bulk (OpenMX elastic paper)',
        'bulk_lattice': DFT_LATTICE,
        'dft_data_placeholder': '__DFT_DATA_PATH__',
        'bulk': {}, 'slabs': [],
    }
    ...
    write_openmx(
        os.path.join(SP_DIR, 'bulk_dft_SP.dat'),
        'bulk_dft_SP',
        'β-Sn bulk SP at DFT/PBE-optimized lattice',
        f'bulk 4 atoms, a={DFT_LATTICE["a"]:.3f}, c={DFT_LATTICE["c"]:.3f}',
        bulk, kb, MD_SP,
    )
    manifest['bulk'] = {'n_atoms': 4, 'kgrid': list(kb), **DFT_LATTICE}
    print(f'SP/bulk_dft_SP.dat  n=4  kgrid={kb}')
    ...
    for tag, L in SLABS:
        ...
        write_openmx(
            os.path.join(RELAX_DIR, sysname + '.dat'),
            sysname,
            f'β-Sn ({tag}) slab L={L} full-atom relaxation at DFT lattice',
            f'slab ({tag}) L={L}, vacuum {VACUUM:.0f} Å, all atoms free',
            slab, kg, MD_RELAX,
        )
        write(os.path.join(RELAX_DIR, f'{sysname}.traj'), slab)
        manifest['slabs'].append({
            'sysname': sysname, 'tag': tag, 'layers': L, 'n_atoms': len(slab),
            'cell_Ang': [Lx, Ly, Lz], 'area_A2': A, 'kgrid': list(kg),
        })
        print(f'Relax/{sysname}.dat  n={len(slab)}  cell=({Lx:.2f},{Ly:.2f},{Lz:.2f})  kgrid={kg}')
